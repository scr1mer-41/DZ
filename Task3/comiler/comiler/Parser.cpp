...
void Parser::compile()
{
    table.openScope(); // Áëîê ñòàíäàðòíûõ èäåíòèôèêàòîðîâ
...
void Parser::modulePrc()
{
    ...
    {
        // IMPORT îïöèîíàëüíûé
        // ðàñïîçíàþùàÿ ïðîöåäóðà äëÿ íåòåðìèíàëà IMPORT
        importPrc();
    }
    ...
    {
        errorPtr->syntaxError("èìÿ ìîäóëÿ");
    }
    else if (item->name != moduleName)
    {
        errorPtr->syntaxError("èìÿ ìîäóëÿ " + scanner.nameValue);
    }
...
void Parser::loccateVariables()
{
    ...
    for (Item& var : vars)
    {
        if (std::stoi(var.addr) > 0) {
            ...
            generateCode.gen(0); // Óâåëè÷èëè cmdCounter íà 1
        }
        else
        {
            std::cout << "Ïåðåìåííà `" << var.name << "` îáúÿâëåíà, íî íå èñïîëüçóåòñÿ" << std::endl;
        }
    }
}
...
int Parser::constExpresionPrc()
{
    ...
    // íå ïðîâåðÿåì äåëåíèå íà 0, ïåðåïîëíåíèå è òï
    ...
    if (scanner.lex == Scanner::Lex::NAME)
    {
        ...
        {
            errorPtr->contextError("èìÿ ìîäóëÿ");
        }
        else
        {
    ...
    {
        errorPtr->syntaxError("èìÿ êîíñòàíòû èëè ÷èñëî");
    }
}
...
void Parser::typePrc()
{
    ...
    {
        errorPtr->contextError("Íåîáüÿâëåííîå èìÿ òèïà");
    }
...
void Parser::variableOrCallPrc()
{
    ...
    if (item->typeOfItem == "var")
    {
        ...
        {
            errorPtr->contextError("Íåâåðíûé òèï ïðè ïðèñâàèâàíèè");
        }
    ...
    else if (item->typeOfItem == "procedure" || item->typeOfItem == "module")
    {
        if (scanner.lex == Scanner::Lex::DOT)
        {
            if (item->typeOfItem != "module")
            {
                errorPtr->contextError("Îæèäàåòñÿ èìÿ ìîäóëÿ");
            }
            scanner.nextLex();
            // checkLex(Scanner::Lex::NAME);
            errorIfNotExpectedLex(Scanner::Lex::NAME);
            ...
            {
                errorPtr->contextError("Îæèäàåòñÿ ïðîöåäóðà");
            }
            scanner.nextLex();
        }
        else if (item->typeOfItem != "procedure")
        {
            errorPtr->contextError("Îæèäàåòñÿ èìÿ ïðîöåäóðû");
        }
        ...
        {
            errorPtr->contextError("Îæèäàåòñÿ ñêîáêà");
        }

    }
    else
    {
        errorPtr->contextError("Îæèäàåòñÿ èìÿ ïåðìåííîé èëè ïðîöåäóðû");
    }
}
...
void Parser::forStatementPrc() {
        ...
        if (varItem->typeOfItem != "var" || varItem->type != Item::ItemTypes::Integer)
            errorPtr->contextError("Îæèäàåòñÿ öåëî÷èñëåííàÿ ïåðåìåííàÿ");
...
// Ïàðàìåòð | Âûðàæåíèå
// Ïîêà íåò êîíòåêñòíîãî àíàëèçàòîðà
void Parser::parameterPrc()
{
    expressionPrc();
}

// ÏðîñòîåÂûðàæ [Îòíîøåíèå ÏðîñòîåÂûðàæ]
...
// Ìíîæèòåëü {ÎïåðàöèÿÓìíîæåíèÿ Ìíîæèòåëü}
...
Item::ItemTypes Parser::multiplierPrc()
{
    if (scanner.lex == Scanner::Lex::NAME)
    {
        ...
        {
            errorPtr->contextError("Îæèäàåòñÿ êîíñòàíòà, èìÿ èëè ôóíêöèÿ");
        }
    }
    else if (scanner.lex == Scanner::Lex::NUM)
    {
    ...
    {
        errorPtr->syntaxError("Èìÿ, ÷èñëî èëè '('");
    }
...
void Parser::contextImportPrc()
{
    ...
    {
        errorPtr->contextError("Îæèäàåòñÿ ìîäóëü `In` èëè `Out`");
    }
...
void Parser::checkProcParameters(Item item)
{
    ...
    {
        errorPtr->contextError("Íåèçâåñòíàÿ ïðîöåäóðà");
    }
}
...
void Parser::checkFuncParameters(Item item)
{
    ...
    {
        errorPtr->contextError("Íåèçâåñòíàÿ ôóíêöèÿ");
    }
}
...
void Parser::checkIntType(Item::ItemTypes type)
{
    ...
    {
        errorPtr->contextError("Îæèäàåòñÿ öåëûé òèï");
    }
}
...
void Parser::checkBoolType(Item::ItemTypes type)
{
    ...
    {
        errorPtr->contextError("Îæèäàåòñÿ ëîãè÷åñêèé òèï");
    }
}
...
void Parser::errorIsNotVariable() {
    ...
    {
        errorPtr->contextError("Îæèäàåòñÿ èìÿ ïåðåìåííîé");
    }
