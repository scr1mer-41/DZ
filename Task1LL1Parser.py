class LL1Parser:
    ...
    def parse(self, text):
        ...
        self.tokens.append(Token("EOF", self.current_char, self.pos))
        return self.tokens
...
class Evaluate:
    ...
    def Calc(self, token):
        ...
        if token.type == 'OPERATOR':
            ...
            if str(token.value).isalpha():
                res = float(input(f"Введите значение переменной {token.value}: "))
                self.limit_on_number(res)
                value = token.value
                for x in self.tokens:
                    if (x.type == "IDENT") and (x.value == value):
                        self.tokens[self.tokens.index(x)] = token
                token.value = res
                self.stack.append(res)
            else:
